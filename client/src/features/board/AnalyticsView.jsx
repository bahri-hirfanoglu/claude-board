            ...
            <div className="flex items-center justify-between mb-0.5">
              <div className="flex items-center gap-2">
                <div
                  ...
                  style={{ backgroundColor: colorMap?.[item.key] || '#DA7756' }}
                />
...
export default function AnalyticsView({ tasks, projectId }) {
  ...
  const sortedTasks = useMemo(() => {
    ...
    return [...analytics.completedTasks].sort((a, b) => {
      ...
      if (sortField === 'total_cost') {
        ...
        va = a.work_duration_ms || 0;
        vb = b.work_duration_ms || 0;
      } else {
        va = a.total_cost || 0;
        vb = b.total_cost || 0;
      }
  ...
  const SortIcon = ({ field }) => {
    ...
    return sortDir === 'desc' ? (
      ...
      <ChevronUp size={10} className="text-claude" />
    );
  };
  ...
  if (!analytics) {
    return (
      <div className="h-full flex items-center justify-center text-surface-500 text-sm">{t('analytics.noData')}</div>
    );
  }
    ...
    <div className="h-full overflow-y-auto p-4 space-y-5">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-2.5">
        <StatCard
          ...
          value={`$${analytics.totalCost.toFixed(3)}`}
          sublabel={`${analytics.total} tasks`}
        ...
        <StatCard
          ...
          sublabel={`${formatTokens(analytics.totalInput)} in / ${formatTokens(analytics.totalOutput)} out`}
          color="text-blue-400"
        ...
        <StatCard
          ...
          value={`$${analytics.avgCost.toFixed(3)}`}
          sublabel={t('analytics.perTask')}
        ...
        <StatCard
          ...
          value={formatTokens(analytics.avgTokens)}
          sublabel={t('analytics.perTask')}
      ...
      <div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-2.5">
          ...
          <div className="p-3 rounded-lg bg-surface-800/50 border border-surface-700/30 text-center">
            ...
            <div
              className={`text-sm font-semibold ${analytics.successRate > 80 ? 'text-emerald-400' : analytics.successRate > 50 ? 'text-amber-400' : 'text-red-400'}`}
            >
            ...
            <table className="w-full text-xs">
              <thead>
                <tr className="border-b border-surface-700/30">
                  ...
                  <th
                    className="text-right px-3 py-2 text-surface-500 font-medium cursor-pointer hover:text-surface-300"
              ...
              <tbody>
                {sortedTasks.slice(0, 20).map((task, i) => {
                  ...
                  return (
                    <tr
                      key={task.id}
                      className={`border-b border-surface-700/20 hover:bg-surface-700/20 ${isTopCost ? 'bg-amber-500/5' : ''}`}
                    >
                      <td className="px-3 py-2">
                        <div className="flex items-center gap-1.5">
                          {task.task_key && (
                            <span className="text-[9px] text-surface-600 font-mono">{task.task_key}</span>
                          )}
                      ...
                      <td className="px-3 py-2">
                        <div className="flex items-center gap-1.5">
                          <div
                            ...
                            style={{ backgroundColor: MODEL_COLORS_HEX[task._model] || '#6b7280' }}
                          />
