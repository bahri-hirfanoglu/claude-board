    ...
    <div className="space-y-1">
      {entries.map((entry) => {
        if (entry.type === 'tool_group') {
          ...
          let statusEl;
          if (!hasResult)
            statusEl = <span className="w-1.5 h-1.5 rounded-full bg-amber-400 animate-pulse flex-shrink-0" />;
          ...
          return (
            ...
            >
              ...
              >
                ...
                {entry.detail && (
                  <span className="text-surface-500 truncate text-[10px] min-w-0 flex-1 font-mono">{entry.detail}</span>
                )}
                ...
                <div className="px-3 pb-2.5 pt-0 text-[10px] space-y-1.5 border-t border-surface-700/20">
                  {entry.detail && (
                    <div className="flex gap-2 mt-1.5">
                      <span className="text-surface-600 w-10 flex-shrink-0 text-[9px] uppercase tracking-wide font-medium">
                        path
                      </span>
                  ...
                  {resultText && (
                    <div className="mt-1">
                      ...
                      <pre
                        className={`rounded-md bg-surface-950/80 border border-surface-700/20 px-2.5 py-2 text-[10px] font-mono overflow-x-auto max-h-[120px] overflow-y-auto whitespace-pre-wrap break-words leading-relaxed ${
        ...
        return (
          <div
            key={entry.index}
            className={`flex items-start gap-2 text-[11px] px-3 py-1.5 ${isError ? 'text-red-400/80' : 'text-surface-500'}`}
          >
            {isError ? (
