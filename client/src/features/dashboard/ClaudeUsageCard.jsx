...
export function setUsageCache(val) {
  usageCache = val;
}
  ...
  if (!u.tasks_with_usage) {
    return (
      <div className="mb-8 rounded-xl bg-gradient-to-br from-surface-800/80 to-surface-900 border border-surface-700/50 overflow-hidden">
        ...
        <div className="p-5 text-center text-surface-600 text-sm py-8">{t('usage.noData')}</div>
      </div>
    );
  }
        ...
        <div className="flex items-center gap-3 ml-auto">
          ...
          {limits?.context_window > 0 && (
            <span className="text-[10px] text-surface-500">
              {formatTokens(limits.context_window)} {t('usage.context')}
            </span>
          )}
      ...
      <div className="p-5">
          ...
          <div>
            ...
            <div className="flex items-center gap-2 mt-1">
              ...
              <span className="text-[9px] text-surface-600">
                {inputPct.toFixed(0)}% {t('usage.input')}
              </span>
            </div>
          </div>
          <div>
            ...
            <div className="text-[10px] text-surface-600 mt-1">
              ~${totalTokens > 0 ? ((u.total_cost || 0) / (totalTokens / 1e6)).toFixed(2) : '0'}
              {t('usage.perMTokens')}
            </div>
          </div>
          <div>
            ...
            <div className="text-[10px] text-surface-600 mt-1">
              ~{u.tasks_with_usage > 0 ? Math.round((u.total_turns || 0) / u.tasks_with_usage) : 0}{' '}
              {t('usage.avgPerTask')}
            </div>
          </div>
          <div>
            ...
            <div
              className={`text-xl font-bold ${(u.rate_limit_hits || 0) > 0 ? 'text-amber-400' : 'text-emerald-400'}`}
            >
              {u.rate_limit_hits || 0}
            </div>
            <div className="text-[10px] text-surface-600 mt-1">
        ...
        {(u.cache_read || 0) > 0 && (
          <div className="mt-4 pt-3 border-t border-surface-700/30 flex items-center gap-4 text-[10px] text-surface-500">
            <span>
              {t('usage.cacheRead')}: {formatTokens(u.cache_read)}
            </span>
            <span>
              {t('usage.cacheCreated')}: {formatTokens(u.cache_creation)}
            </span>
            <span className="text-emerald-500/70">
              {totalTokens > 0
                ? `${((u.cache_read / (totalTokens + u.cache_read)) * 100).toFixed(0)}% ${t('usage.cacheHit')}`
                : ''}
            </span>
          </div>
        )}
