...
function ScanButton({ projectId, t }) {
  ...
  useEffect(() => {
    ...
    const close = (e) => { if (choiceRef.current && !choiceRef.current.contains(e.target)) setShowChoice(false); };
    window.addEventListener('mousedown', close);
    return () => window.removeEventListener('mousedown', close);
  }, [showChoice]);
  ...
  const doScan = async (mode) => {
    ...
    setDone(false);
    try {
      await api.scanCodebase(projectId, mode);
      setDone(true);
  ...
  return (
    <div className="relative">
      ...
      {showChoice && (
        <div ref={choiceRef} className="absolute right-0 top-full mt-1 w-56 bg-surface-800 border border-surface-700 rounded-lg shadow-xl z-50 py-1">
          <button onClick={() => doScan('overwrite')} className="w-full px-3 py-2 text-left text-xs hover:bg-surface-700 text-surface-300 flex items-center gap-2">
            <ScanSearch size={12} className="text-blue-400" />
            <div><div className="font-medium">{t('header.scanOverwrite')}</div><div className="text-[10px] text-surface-500">{t('header.scanOverwriteDesc')}</div></div>
          </button>
          <button onClick={() => doScan('append')} className="w-full px-3 py-2 text-left text-xs hover:bg-surface-700 text-surface-300 flex items-center gap-2">
            <Plus size={12} className="text-emerald-400" />
            <div><div className="font-medium">{t('header.scanAppend')}</div><div className="text-[10px] text-surface-500">{t('header.scanAppendDesc')}</div></div>
          </button>
        </div>
      )}
    </div>
