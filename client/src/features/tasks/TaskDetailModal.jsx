...
export default function TaskDetailModal({ task, onClose }) {
  ...
  const TYPE_COLORS = {
    ...
    refactor: 'bg-purple-500/15 text-purple-400', docs: 'bg-green-500/15 text-green-400',
    test: 'bg-yellow-500/15 text-yellow-400', chore: 'bg-surface-500/15 text-surface-400',
  };
  const STATUS_COLORS = {
    backlog: 'text-surface-400', in_progress: 'text-amber-400', testing: 'text-claude', done: 'text-emerald-400',
  };

  return (
    <div className="fixed inset-0 bg-black/60 flex items-start justify-center z-50 p-4 pt-[5vh] overflow-y-auto" onClick={onClose}>
      <div className="bg-surface-900 rounded-xl border border-surface-700 w-full max-w-2xl shadow-2xl animate-slide-up" onClick={e => e.stopPropagation()}>
        {/* Header */}
        <div className="flex items-start justify-between px-5 py-4 border-b border-surface-800">
          <div className="min-w-0 flex-1">
            <div className="flex items-center gap-2 mb-1 flex-wrap">
              ...
              <span className={`text-[10px] font-medium ${STATUS_COLORS[d.status]}`}>{d.status?.replace('_', ' ')}</span>
              <span className="text-[10px] text-surface-600">#{d.id}</span>
              {d.revision_count > 0 && <span className="text-[10px] font-medium px-1.5 py-0.5 rounded bg-amber-500/15 text-amber-400">Rev {d.revision_count}</span>}
            </div>
          ...
          <button onClick={onClose} className="p-1 rounded-lg hover:bg-surface-800 text-surface-400 ml-3 flex-shrink-0"><X size={16} /></button>
        </div>

        {loading ? (
          <div className="flex items-center justify-center py-16"><div className="w-5 h-5 rounded-full border-2 border-claude/20 border-t-claude animate-spin" /></div>
        ) : (
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
              {duration && (
                <div className="bg-surface-800/50 rounded-lg px-3 py-2">
                  <div className="flex items-center gap-1 text-[10px] text-surface-500 mb-0.5"><Clock size={9} />Duration</div>
                  <div className="text-sm font-semibold text-surface-200">{duration}</div>
                </div>
              )}
              {totalTokens > 0 && (
                <div className="bg-surface-800/50 rounded-lg px-3 py-2">
                  <div className="flex items-center gap-1 text-[10px] text-surface-500 mb-0.5"><Cpu size={9} />Tokens</div>
                  <div className="text-sm font-semibold text-surface-200">{formatTokens(totalTokens)}</div>
                  <div className="text-[9px] text-surface-600">{(d.input_tokens || 0).toLocaleString()} in / {(d.output_tokens || 0).toLocaleString()} out</div>
                </div>
              )}
              {d.total_cost > 0 && (
                <div className="bg-surface-800/50 rounded-lg px-3 py-2">
                  <div className="flex items-center gap-1 text-[10px] text-surface-500 mb-0.5"><Coins size={9} />Cost</div>
                  <div className="text-sm font-semibold text-surface-200">${d.total_cost.toFixed(4)}</div>
                </div>
              )}
              {d.num_turns > 0 && (
                <div className="bg-surface-800/50 rounded-lg px-3 py-2">
                  <div className="flex items-center gap-1 text-[10px] text-surface-500 mb-0.5"><Activity size={9} />Turns</div>
                  ...
                  {revisions.map(rev => (
                    <div key={rev.id} className="bg-surface-800/40 rounded-lg px-3 py-2">
                      <div className="flex items-center gap-2 mb-1">
                        <span className="text-[10px] font-medium text-amber-400 bg-amber-500/10 px-1.5 py-0.5 rounded">Rev #{rev.revision_number}</span>
