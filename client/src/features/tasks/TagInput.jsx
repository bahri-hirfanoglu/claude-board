...
export default function TagInput({ value = [], onChange, suggestions = [], placeholder = 'Add tag...' }) {
  ...
  return (
    <div className="relative">
      <div
        className="flex flex-wrap gap-1 items-center bg-surface-800/50 border border-surface-700/30 rounded-lg px-2 py-1.5 min-h-[32px]"
      ...
      >
        {value.map((tag) => {
          ...
          return (
            <span
              key={tag}
              className={`inline-flex items-center gap-0.5 text-[10px] font-medium px-1.5 py-0.5 rounded ${color}`}
            >
              {tag}
              <button
                onClick={(e) => {
