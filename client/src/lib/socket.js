...
if (IS_TAURI) {
  ...
  import('@tauri-apps/api/event').then(({ listen }) => {
    const events = [
      'task:created', 'task:updated', 'task:deleted', 'task:usage', 'task:log',
      'task:attachments', 'task:attachmentDeleted',
      'project:created', 'project:updated', 'project:deleted',
      'snippet:created', 'snippet:updated', 'snippet:deleted',
      'template:created', 'template:updated', 'template:deleted',
      'role:created', 'role:updated', 'role:deleted',
      'plan:started', 'plan:log', 'plan:phase', 'plan:progress',
      'plan:stats', 'plan:completed', 'plan:cancelled',
      'claude:finished', 'claude:limits',
    ];
  ...
  socket = {
    connected: true,
    on(event, callback) {
      if (event === 'connect') { setTimeout(callback, 0); return; }
    ...
    off(event, callback) {
      ...
      if (cbs) { if (callback) cbs.delete(callback); else listeners.delete(event); }
    },
    emit() {},
  };
} else {
