...
pub fn get_project_stats(db: &DbPool, pid: i64) -> ProjectStats {
    ...
    let by_status = {
        let mut s = conn.prepare("SELECT status,COUNT(*) as count FROM tasks WHERE project_id=?1 GROUP BY status").unwrap();
        s.query_map(params![pid], |r| Ok(CountRow { label: r.get(0)?, count: r.get(1)? }))
            .unwrap().flatten().collect()
    };

    let by_priority = {
        let mut s = conn.prepare("SELECT priority,COUNT(*) as count FROM tasks WHERE project_id=?1 GROUP BY priority").unwrap();
        s.query_map(params![pid], |r| Ok(CountRow { label: format!("{}", r.get::<_, i64>(0)?), count: r.get(1)? }))
            .unwrap().flatten().collect()
    };

    let by_type = {
        let mut s = conn.prepare("SELECT task_type,COUNT(*) as count FROM tasks WHERE project_id=?1 GROUP BY task_type").unwrap();
        s.query_map(params![pid], |r| Ok(CountRow { label: r.get(0)?, count: r.get(1)? }))
            .unwrap().flatten().collect()
    };

    let duration = conn.prepare(
    ...
    .query_row(params![pid], |r| Ok(DurationStats {
    ...
    let timeline = {
        ...
        s.query_map(params![pid], |r| Ok(TimelinePoint { day: r.get(0)?, count: r.get(1)? }))
            .unwrap().flatten().collect()
    };
    ...
    let recent_completed = {
        ...
        s.query_map(params![pid], |r| Ok(RecentCompleted {
    ...
    .query_row(params![pid], |r| Ok(ClaudeUsage {
    ...
    let raw_breakdown: Vec<ModelBreakdown> = {
        ...
        s.query_map(params![pid], |r| Ok(ModelBreakdown {
...
fn merge_model_breakdown(rows: Vec<ModelBreakdown>) -> Vec<ModelBreakdown> {
    ...
    for row in rows {
        ...
        *entry.total_tokens.as_mut().unwrap() += row.total_tokens.unwrap_or(0);
        *entry.total_cost.as_mut().unwrap() += row.total_cost.unwrap_or(0.0);
    }
...
pub fn get_global_usage(db: &DbPool) -> GlobalUsage {
    let conn = db.lock();
    let mut stmt = conn.prepare("SELECT SUM(COALESCE(input_tokens,0)),SUM(COALESCE(output_tokens,0)),SUM(COALESCE(cache_read_tokens,0)),SUM(COALESCE(cache_creation_tokens,0)),SUM(COALESCE(total_cost,0)),SUM(COALESCE(num_turns,0)),SUM(COALESCE(rate_limit_hits,0)),COUNT(CASE WHEN input_tokens>0 THEN 1 END),COUNT(*) FROM tasks").unwrap();
...
pub fn get_global_model_breakdown(db: &DbPool) -> Vec<GlobalModelRow> {
    ...
    let raw: Vec<GlobalModelRow> = stmt.query_map([], |r| Ok(GlobalModelRow {
    ...
    for row in raw {
        ...
        *entry.input_tokens.as_mut().unwrap() += row.input_tokens.unwrap_or(0);
        *entry.output_tokens.as_mut().unwrap() += row.output_tokens.unwrap_or(0);
        *entry.cost.as_mut().unwrap() += row.cost.unwrap_or(0.0);
    }
    map.into_values().collect()
}
...
pub fn get_claude_limits(db: &DbPool) -> Option<ClaudeLimits> {
    let conn = db.lock();
    let mut stmt = conn.prepare("SELECT * FROM claude_limits WHERE id=1").unwrap();
...
pub fn get_usage_timeline(db: &DbPool) -> Vec<UsageTimelinePoint> {
    ...
    stmt.query_map([], |r| Ok(UsageTimelinePoint {
