...
pub fn get_by_project(db: &DbPool, pid: i64) -> Vec<Webhook> {
    let conn = db.lock();
    let mut stmt = conn.prepare("SELECT * FROM webhooks WHERE project_id=?1 ORDER BY created_at DESC").unwrap();
    stmt.query_map(params![pid], |r| row_to(r)).unwrap().flatten().collect()
}

pub fn get_by_id(db: &DbPool, id: i64) -> Option<Webhook> {
    let conn = db.lock();
    let mut stmt = conn.prepare("SELECT * FROM webhooks WHERE id=?1").unwrap();
    stmt.query_row(params![id], |r| row_to(r)).ok()
}

pub fn get_enabled_by_project(db: &DbPool, pid: i64) -> Vec<Webhook> {
    let conn = db.lock();
    let mut stmt = conn.prepare("SELECT * FROM webhooks WHERE project_id=?1 AND enabled=1").unwrap();
    stmt.query_map(params![pid], |r| row_to(r)).unwrap().flatten().collect()
}
...
pub fn delete(db: &DbPool, id: i64) {
    let conn = db.lock();
    conn.execute("DELETE FROM webhooks WHERE id=?1", params![id]).unwrap();
}
