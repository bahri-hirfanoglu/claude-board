...
pub fn start_planning(
    ...
    std::thread::spawn(move || {
        ...
        let tasks_parsed = parse_tasks_from_output(&full_text);
        let db = db::get_db();

        if !tasks_parsed.is_empty() {
            ...
            activity::add(&db, project_id, None, "plan_completed",
                &format!("Planning completed: {} tasks from \"{}\"", created.len(), topic_clone.trim()), None);
            app.emit("plan:completed", &serde_json::json!({
                "projectId": project_id, "planId": &plan_id_clone,
                "tasks": created,
...
fn build_planning_prompt(topic: &str, context: &str, project: &pq::Project, granularity: &str) -> String {
    ...
    format!(r#"You are a technical project planner. Analyze the following topic and create a structured task breakdown.
...
Rules:
- Create {} tasks
- Descriptions should be detailed enough for Claude to implement"#,
