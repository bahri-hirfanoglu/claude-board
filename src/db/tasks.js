...
export const queries = {
  // CRUD
  getTasksByProject: { all: (pid) => queryAll('SELECT * FROM tasks WHERE project_id=? ORDER BY status,sort_order,id', [pid]) },
  getTaskById: { get: (id) => queryOne('SELECT * FROM tasks WHERE id=?', [id]) },
  createTask: {
    run: (pid, title, desc, priority, type, criteria, model, effort) => run(
  ...
  updateTask: {
    run: (title, desc, priority, type, criteria, model, effort, id) => run(
  ...
  // Usage tracking
  setTaskUsageLive: {
    run: (input, output, cacheRead, cacheCreation, cost, model, id) => run(
  ...
  // Revisions
  addRevision: { run: (tid, num, feedback) => run('INSERT INTO task_revisions (task_id,revision_number,feedback) VALUES (?,?,?)', [tid, num, feedback]) },
  getRevisions: { all: (tid) => queryAll('SELECT * FROM task_revisions WHERE task_id=? ORDER BY revision_number ASC', [tid]) },
  ...
  // Queue
  getNextQueuedTask: (pid) => queryOne("SELECT * FROM tasks WHERE project_id=? AND status='backlog' ORDER BY priority DESC,queue_position ASC,id ASC LIMIT 1", [pid]),
  getRunningCount: (pid) => { const r = queryOne("SELECT COUNT(*) as count FROM tasks WHERE project_id=? AND status='in_progress'", [pid]); return r?.count || 0; },
};
