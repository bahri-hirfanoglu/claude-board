...
export default function statsRoutes({ projectQueries, statsQueries, activityLog }) {
  ...
  router.get('/projects/:projectId/stats', asyncHandler(async (req, res) => {
  ...
  router.get('/stats/claude-usage', asyncHandler(async (req, res) => {
    res.json({
  ...
  router.get('/projects/:projectId/claude-md', asyncHandler(async (req, res) => {
    const project = projectQueries.getById(req.params.projectId);
    if (!project) return res.status(404).json({ error: 'Project not found' });
  ...
  router.put('/projects/:projectId/claude-md', asyncHandler(async (req, res) => {
    const project = projectQueries.getById(req.params.projectId);
    if (!project) return res.status(404).json({ error: 'Project not found' });
